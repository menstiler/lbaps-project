  ...
  .superRefine((data, ctx) => {
    ...
    if (!hasFieldName) {
      ctx.addIssue({
        code: "custom",
        message:
          "Field name is required when either field type or field value is provided.",
        path: ["field_name"],
      });
    }
    ...
    if (!hasFieldType) {
      ctx.addIssue({
        code: "custom",
        message:
          "This field is required when either field name or field value is provided.",
        path: ["field_type"],
      });
    }
    ...
    if (!hasFieldValue) {
      ctx.addIssue({
        code: "custom",
        message:
          "This field is required when either field name or field type is provided.",
        path: ["field_value"],
      });
    }
    ...
    if (hasFieldName && hasFieldType && hasFieldValue) {
      if (
        ...
        typeof data.field_value !== "string"
      ) {
        ctx.addIssue({
          code: "custom",
